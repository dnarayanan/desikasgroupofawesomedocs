...
So now let's pretend we observed this system with JWST. For this z=6.5 galaxy, restframe RGB falls into MIRI's 770W filter and NIRCam's 444W and 277W filters. We can "observe" this galaxy with JWST by convolving the images with each filter's pixel scale::
  ...
  new_size = int(pd_pixel_scale/spitzer_pixel_scale * np.shape(convolved_image_g)[0])
  g_rescaled = rescale(convolved_image_g, new_size/512, anti_aliasing=True)

  new_size = int(pd_pixel_scale/spitzer_pixel_scale * np.shape(convolved_image_r)[0])
  r_rescaled = rescale(convolved_image_r, new_size/512, anti_aliasing=True)

  new_size = int(pd_pixel_scale/spitzer_pixel_scale * np.shape(convolved_image_b)[0])
  b_rescaled = rescale(convolved_image_b, new_size/512, anti_aliasing=True)
...
Finally, let's gain a greater appreciation for JWST's power by seeing what Spitzer would've seen::
  ...
  #IRAC PSF
  psf_irac_ch1 = fits.open('/orange/narayanan/s.lower/simba/m25n256_dm/zooms/pd_runs/imaging/run2_snap065/spitzer_psf/irac_ch1/apex_sh_IRAC1_col129_row025_x100.fits')
  psf_irac_ch2 = fits.open('/orange/narayanan/s.lower/simba/m25n256_dm/zooms/pd_runs/imaging/run2_snap065/spitzer_psf/irac_ch2/apex_sh_IRAC2_col129_row129_x100.fits')
  psf_irac_ch4 = fits.open('/orange/narayanan/s.lower/simba/m25n256_dm/zooms/pd_runs/imaging/run2_snap065/spitzer_psf/irac_ch4/apex_sh_IRAC4_col129_row233_x100.fits')
